...
if (0 != $item) {
    $GLOBALS['xoopsOption']['template_main'] = 'enterprise_service.html';

    $service = Enterprise::get_service($item);

    $xoopsTpl->assign('service', $service);
} else {
    $GLOBALS['xoopsOption']['template_main'] = 'enterprise_services.html';

    $nav = '<a href="' . XOOPS_URL . '/modules/enterprise/">Enterprise Home</a>->Services';

    $intro = Enterprise::get_servicesIntro();

    $services = Enterprise::get_services();
